"""
Full pipeline: N agents from descriptions -> 384D persona vectors -> network.py.

Flow:
1. Load NUM_AGENTS agent descriptions (from hybrid_review_dataset or fallback).
2. Compute 384D personality vectors via preprocessor(embed(description)).
3. Run network.run_media_pipeline with NUM_AGENTS UIDs and vectors.
...
- hybrid_review_dataset.json (descriptions are reused when fewer than NUM_AGENTS are available)
"""
...
def get_100_vectors(
    ...
    Returns (description_vectors, personality_vectors) each (NUM_AGENTS, 384) float64.
...
def main(
    ...
    # 2. NUM_AGENTS agents: random draw each run; pipeline seed still controls network/sharing
    ...
    # 3. Batch compute all responses (temperature sampling so we don't always get "dislike")
    ...
    print("=" * 60)
    print(f"FULL PIPELINE: {NUM_AGENTS} agents -> network")
    print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description=f"Run full pipeline: {NUM_AGENTS} agents -> network (10%% exposure, reactions, sharing)")
