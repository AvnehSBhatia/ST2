...
def sample_top_result_with_neutral(
    ...
    counts = Counter(label for _, label, _ in top_results)
    pos_count = sum(counts.get(label, 0) for label in POSITIVE_SENTIMENTS)
    neg_count = sum(counts.get(label, 0) for label in NEGATIVE_SENTIMENTS)
    # Explicit top-k split rule requested by the dashboard simulation:
    # if the top-5 nearest answers split 2/3 or 3/2 across positive/negative,
    # treat the agent as neutral and take no action.
    if {pos_count, neg_count} == {2, 3}:
        return ("I feel neutral. I can see both sides.", "neutral", 0.0), counts
...
def resolve_batch_topk_results(
    similarity_rows: torch.Tensor,
    bank: dict,
    top_k: int = 5,
    threshold: float = NEUTRAL_TIE_THRESHOLD,
) -> list[tuple[str, str, float, Counter]]:
    """
    Resolve a batch of cosine-similarity rows against an answer bank using the same
    top-k + neutral fallback logic as the single-example CLI flow.
    """
    if similarity_rows.ndim != 2:
        raise ValueError("similarity_rows must be a 2D tensor of shape (batch, bank_size)")

    if not bank["answer_texts"]:
        return [("No matching answers found.", "neutral", 0.0, Counter()) for _ in range(similarity_rows.size(0))]

    k = min(top_k, similarity_rows.size(1))
    topk = similarity_rows.topk(k, dim=1)
    resolved: list[tuple[str, str, float, Counter]] = []
    for row_indices, row_scores in zip(topk.indices.tolist(), topk.values.tolist()):
        top_results = [
            (
                bank["answer_texts"][idx],
                bank["sentiment_labels"][idx],
                float(score),
            )
        ...
        resolved.append((answer_text, sentiment_label, score, counts))
    return resolved
